  ...
  const render = () => {
    ...
    document.querySelector('#music-import-file').onchange = async event => {
      ...
      try {
        ...
        preview.innerHTML = `<div class="alert alert-success py-2">${songs.length} música(s) encontrada(s).${duplicates ? ` ${duplicates} já cadastrada(s) foram desmarcadas.` : ' Revise e confirme a importação.'}</div><div class="list-group">${songs.map((song, index) => `<label class="list-group-item ${song.alreadyImported ? 'list-group-item-light text-secondary' : ''}"><input class="form-check-input me-2 import-song" type="checkbox" data-index="${index}" ${song.alreadyImported ? 'disabled' : 'checked'}><strong>${UI.esc(song.title)}</strong>${song.alreadyImported ? ' <span class="badge text-bg-secondary ms-1">Já cadastrada</span>' : song.artist ? ` <span class="text-secondary">— ${UI.esc(song.artist)}</span>` : ''}<small class="d-block text-secondary text-truncate">${UI.esc(song.lyrics)}</small></label>`).join('')}</div>`;
        confirm.disabled = songs.every(song => song.alreadyImported);
      } catch (error) { preview.innerHTML = `<div class="alert alert-danger mb-0">${UI.esc(error.message)}</div>`; }
    };
    document.querySelector('#confirm-music-import').onclick = async () => {
      const selected = [...document.querySelectorAll('.import-song:checked')].map(input => songs[Number(input.dataset.index)]);
      if (!selected.length) return UI.alert('Selecione pelo menos uma música.', 'warning');
      try { const result = await API.post('/music/import/confirm', { songs: selected }); modal.hide(); UI.alert(`${result.imported} música(s) importada(s).`); await music(); }
