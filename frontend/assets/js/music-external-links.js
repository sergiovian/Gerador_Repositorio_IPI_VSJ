document.addEventListener('DOMContentLoaded', () => {
  ...
  function openLyricsBrowser(music) {
    ...
    browser.innerHTML = `<div class="chord-browser-card"><header><strong><i class="bi bi-search me-2"></i>Pesquisar letra: ${esc(music.title)}</strong><button type="button" class="btn-close chord-browser-close" aria-label="Voltar ao editor"></button></header><div class="chord-browser-help">Copie a letra no site e clique em <strong>Voltar ao editor</strong> para colar na cifra. Se o site não permitir abrir aqui, use <a href="${url}" target="_blank" rel="noopener">abrir em nova aba</a>; seu editor continuará aberto.</div><iframe src="${url}" title="Busca de letras" referrerpolicy="no-referrer"></iframe></div>`;
    document.body.append(browser); browser.querySelector('.chord-browser-close').onclick = () => browser.remove();
  }
