if (document.documentElement.dataset.page === 'dashboard') {
  const observer = new MutationObserver(() => {
    ...
    API.get('/dashboard/summary').then(data => {
      ...
      const bars = mostUsed.length ? mostUsed.map(item => `<div class="mb-3"><div class="d-flex justify-content-between gap-3 small mb-1"><span class="text-truncate">${UI.esc(item.title)}</span><strong>${item.uses}x</strong></div><div class="progress" style="height:10px"><div class="progress-bar" style="width:${Math.round(item.uses / maximum * 100)}%;background:#7d1e39"></div></div></div>`).join('') : '<p class="text-secondary mb-0">Ainda não há músicas executadas.</p>';
      app.insertAdjacentHTML('beforeend', `<div class="row g-4 mt-1"><div class="col-lg-7"><div class="card panel p-4 h-100"><h2 class="h5 mb-4"><i class="bi bi-bar-chart-line me-2"></i>Músicas mais tocadas</h2>${bars}</div></div><div class="col-lg-5"><div class="card panel p-4 h-100"><h2 class="h5">Sugestão rápida</h2><p class="text-secondary small">Sorteie uma música sem regras de repertório.</p><div class="d-grid gap-2"><button class="btn btn-outline-primary" data-quick-type="HINO"><i class="bi bi-book me-1"></i>Sugerir hino</button><button class="btn btn-primary" data-quick-type="LOUVOR"><i class="bi bi-music-note-beamed me-1"></i>Sugerir louvor</button></div><div id="quick-suggestion" class="mt-3"></div></div></div></div>`);
      app.querySelectorAll('[data-quick-type]').forEach(button => button.onclick = async () => {
        ...
        const picked = music[Math.floor(Math.random() * music.length)];
