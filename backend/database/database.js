...
async function initializeDatabase() {
  ...
  initializationPromise = (async () => {
    try {
      ...
      await executeSql("CREATE TABLE IF NOT EXISTS projection_states (repertoire_id INTEGER PRIMARY KEY, current_position INTEGER NOT NULL DEFAULT 1, blackout INTEGER NOT NULL DEFAULT 0, mode TEXT NOT NULL DEFAULT 'MUSIC', liturgy_index INTEGER NOT NULL DEFAULT 0, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(repertoire_id) REFERENCES repertoires(id) ON DELETE CASCADE)", 'a tabela de projeção');
      ...
      if (!projectionColumns.some((column) => column.name === 'mode')) await executeSql("ALTER TABLE projection_states ADD COLUMN mode TEXT NOT NULL DEFAULT 'MUSIC'", 'o modo da projeção');
      if (!projectionColumns.some((column) => column.name === 'liturgy_index')) await executeSql('ALTER TABLE projection_states ADD COLUMN liturgy_index INTEGER NOT NULL DEFAULT 0', 'a página da liturgia');
